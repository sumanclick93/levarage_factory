...
// Fetch the logged-in user's data for the Root Node
...
// Populate volume maps starting from the logged-in user downward
...
function renderDownlineTreeMemory($parent_id, $level = 1) {
    ...
    // Branches deeper than level 1 are hidden by default to keep the UI clean
...
<head>
    ...
    <title>Network Tree | Leverage Factory</title>
    ...
    <script>
        ...
        /**
         * SEARCH PROTOCOL: Multi-criteria with Result Counter
         */
        function searchTree() {
            ...
            nodes.forEach(node => {
                ...
                if (uid === input || username.includes(input) || email.includes(input)) {
                    ...
                    // Auto-Expand parents to reveal match
            ...
            if (matchCount === 0 && input !== "") {
                counter.innerText = "No Protocol Matches";
                counter.classList.remove('hidden');
                counter.classList.replace('bg-[#00A6FB]', 'bg-red-500');
            } else {
                counter.classList.replace('bg-red-500', 'bg-[#00A6FB]');
            }
        }
    </script>
</head>
    ...
    <main class="flex-1 p-8 overflow-x-auto relative">
        <header class="mb-12 sticky left-0 z-40 bg-[#F8FAFC]/80 backdrop-blur-md py-4">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 max-w-7xl">
                <h1 class="text-4xl font-black uppercase italic tracking-tighter">Network <span class="text-[#00A6FB]">Tree</span></h1>
                <div class="flex flex-col items-end gap-2">
                    ...
                    <div id="matchCounter" class="hidden px-4 py-1.5 bg-[#00A6FB] text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg transition-all animate__animated animate__fadeInRight">
                        0 Matches Found
                    </div>
                </div>
            </div>
        ...
        <div id="network-root" class="inline-block bg-black p-8 rounded-[3rem] shadow-2xl border border-white/10 mb-12">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 bg-[#00A6FB] rounded-[1.5rem] flex items-center justify-center text-white font-black text-xl shadow-[0_0_30px_rgba(0,166,251,0.3)]">YOU</div>
                <p class="text-white text-2xl font-black uppercase italic tracking-tighter"><?php echo htmlspecialchars($logged_user['username']); ?></p>
            </div>
        </div>
