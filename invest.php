...
// We only need ONE query that includes the total_purchased subquery
$stmt = $pdo->query("
    SELECT s.*, 
    (SELECT COUNT(*) FROM investments i WHERE i.scheme_id = s.id AND i.status = 'active') as total_purchased
    FROM investment_schemes s 
    WHERE s.is_active = 1 AND s.type = 'individual'
    ORDER BY s.total_return_percent DESC
");

// Store the results in the variable expected by your HTML loop
                    ...
                    <div class="card-header">
                        <div class="profile">
                            ...
                            <div>
                                ...
                                <div class="followers"><?php echo number_format($s['total_purchased']); ?> / 500 Copiers</div>
                            </div>
                        </div>
                        <div class="actions">
                    ...
                    <div class="card-body">
                        <div>
                            ...
                            <div class="value" style="display: flex; align-items: center; justify-content: space-between;">
                                <span>$<?php echo number_format($s['min_amount'], 0); ?></span>
                                
                                <?php if (strcasecmp(trim($s['name']), 'RENAISSANCE') === 0): ?>
                                    <img src="infinity_icon.jpeg" 
                                         title="Infinity Bonus" 
                                         alt="Infinity Bonus" 
                                         style="width: 90px; height: 50px; cursor: help; object-fit: contain;">
                                <?php endif; ?>
                            </div>
                        </div>
